      ...
      <template v-for="item in menuList">
        <template v-if="item.children && item.children.length === 1">
          ...
          <menu-item v-else :name="getNameOrHref(item, true)" :key="`menu-${item.children[0].name}`">
						<!--处理自定义图标的显示-->
						<common-icon type="''" :class="`icon iconfont margin-right-10 ${item.children[0].icon}`" v-if="item.children[0].icon&&item.children[0].icon.indexOf('icon')!==-1"/>
						<common-icon :type="item.children[0].icon || ''" v-else/>
